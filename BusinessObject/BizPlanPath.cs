...
namespace BusinessObject
{
    public class BizPlanPath
    {
        ...
        {
            var p = new ShortestTimePath(flag);
            var t = new TransferLestPath(flag);
        ...
        public bool ValideData(string originId, string destId, ref string ret, params string[] flag)
        {
            ...
            for (var i = 0; i < DalStations.galPathExStations.Count; i++)
            {
                ...
                for (var j = 0; j < station.EdgeList.Count; j++)
                {
                    var e = station.EdgeList[j] as Edge;
                    if (e.IsStep)
